...
import downloader
import file
from configuration import DEFAULT_EXTERNAL_DOWNLOADER, AUDIO_FILE_TYPE, DEFAULT_DURATION, DEFAULT_LOW_VOLUME_THRESHOLD, DEFAULT_CLIP_LIMIT, DEFAULT_SAMPLE_RATE, CACHE_ROOT, METADATA_FILE_TYPE, DEFAULT_FADE_IN_DURATION, DEFAULT_FADE_OUT_DURATION, EXPORT_ROOT
...
class AudioProcessor:
    ...
    def slice(self, slicers, duration=DEFAULT_DURATION, low_volume_threshold=DEFAULT_LOW_VOLUME_THRESHOLD, clip_limit=DEFAULT_CLIP_LIMIT):
        """
        ...
        def register(remaining_slicers):
            ...
            if isinstance(remaining_slicers, tuple):
                ...
                def registered_slicer(rate, duration, threshold, samples, count, *args, **kwargs):
                    ...
                    return slicer.get_clips()  # return the clips generated by the registered slicer method
        ...
        if 0 == len(slicer_names):
            raise RuntimeError("You didn't provide any slicers")

        self.logger.debug("Clipifying the recording")

        for slicer_name in slicer_names:
            self.clips.extend(getattr(pydub.AudioSegment, slicer_name)(self.recording.frame_rate, duration, low_volume_threshold, self.recording.get_array_of_samples(), clip_limit))

        self.logger.debug(f"Sliced {len(self.clips)} clips from the recording")
